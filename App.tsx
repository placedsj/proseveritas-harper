  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row font-sans selection:bg-red-100 selection:text-red-900">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-2 flex justify-around z-50 overflow-x-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button onClick={() => setView('dashboard')} aria-label="Dashboard" className={`p-2 min-w-[50px] ${view === 'dashboard' ? 'text-red-600' : 'text-slate-400'}`}><LayoutDashboard className="w-6 h-6 mx-auto" /></button>
        <button onClick={() => setView('discovery-archive')} aria-label="Discovery Archive" className={`p-2 min-w-[50px] ${view === 'discovery-archive' ? 'text-indigo-600' : 'text-slate-400'}`}><Database className="w-6 h-6 mx-auto" /></button>
        <button onClick={() => setView('harper-log')} aria-label="Harper Log" className={`p-2 min-w-[50px] ${view === 'harper-log' ? 'text-pink-600' : 'text-slate-400'}`}><Heart className="w-6 h-6 mx-auto" /></button>
        <button onClick={() => setView('system-audit')} aria-label="System Audit" className={`p-2 min-w-[50px] ${view === 'system-audit' ? 'text-blue-600' : 'text-slate-400'}`}><Fingerprint className="w-6 h-6 mx-auto" /></button>
        <button onClick={() => setView('scott-schedule')} aria-label="Scott Schedule" className={`p-2 min-w-[50px] ${view === 'scott-schedule' ? 'text-red-600' : 'text-slate-400'}`}><Scale className="w-6 h-6 mx-auto" /></button>
      </nav>
