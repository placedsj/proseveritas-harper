    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row font-sans selection:bg-red-100 selection:text-red-900">
      ...
      <nav className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-24 bg-white border-r border-slate-200 py-6 items-center z-50 overflow-y-auto scrollbar-hide shadow-sm">
        ...
        <div className="mt-auto py-6 space-y-4 flex flex-col items-center flex-shrink-0">
          <button onClick={toggleSearch} className="p-2 text-slate-400 hover:text-slate-900 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2" aria-label="Open global search" title="Open global search">
      ...
      <header className="md:hidden bg-white p-4 border-b border-slate-200 flex items-center justify-between sticky top-0 z-40 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <button onClick={toggleSearch} className="text-slate-400 hover:text-slate-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2" aria-label="Open global search" title="Open global search">
            <Search className="w-6 h-6" aria-hidden="true" />
          </button>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-2 flex justify-around z-50 overflow-x-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button onClick={() => setView('dashboard')} title="Dashboard" aria-current={view === 'dashboard' ? 'page' : undefined} className={`p-2 min-w-[50px] focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 rounded-lg ${view === 'dashboard' ? 'text-red-600' : 'text-slate-400'}`} aria-label="Dashboard"><LayoutDashboard className="w-6 h-6 mx-auto" aria-hidden="true" /></button>
        <button onClick={() => setView('discovery-archive')} title="Discovery Archive" aria-current={view === 'discovery-archive' ? 'page' : undefined} className={`p-2 min-w-[50px] focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 rounded-lg ${view === 'discovery-archive' ? 'text-indigo-600' : 'text-slate-400'}`} aria-label="Discovery Archive"><Database className="w-6 h-6 mx-auto" aria-hidden="true" /></button>
        <button onClick={() => setView('harper-log')} title="Harper Log" aria-current={view === 'harper-log' ? 'page' : undefined} className={`p-2 min-w-[50px] focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 rounded-lg ${view === 'harper-log' ? 'text-pink-600' : 'text-slate-400'}`} aria-label="Harper Log"><Heart className="w-6 h-6 mx-auto" aria-hidden="true" /></button>
        <button onClick={() => setView('system-audit')} title="System Audit" aria-current={view === 'system-audit' ? 'page' : undefined} className={`p-2 min-w-[50px] focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 rounded-lg ${view === 'system-audit' ? 'text-blue-600' : 'text-slate-400'}`} aria-label="System Audit"><Fingerprint className="w-6 h-6 mx-auto" aria-hidden="true" /></button>
        <button onClick={() => setView('scott-schedule')} title="Scott Schedule" aria-current={view === 'scott-schedule' ? 'page' : undefined} className={`p-2 min-w-[50px] focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 rounded-lg ${view === 'scott-schedule' ? 'text-red-600' : 'text-slate-400'}`} aria-label="Scott Schedule"><Scale className="w-6 h-6 mx-auto" aria-hidden="true" /></button>
      </nav>
