      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-xl border border-blue-200 animate-fade-in shadow-lg text-left">
          <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
             ...
             <button onClick={() => setIsAdding(false)} aria-label="Close form" className="text-slate-400 hover:text-slate-900"><X className="w-5 h-5" /></button>
          </div>
          ...
          <button
            ...
            className={`w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded font-bold shadow-md transition-all active:scale-95 flex justify-center items-center gap-2 uppercase tracking-widest ${(!newLog.action || !newLog.note) ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
