  ...
  useEffect(() => {
    ...
    const loadStats = () => {
      try {
        ...
        if (evidenceStr) {
          verifiedExhibits = JSON.parse(evidenceStr).filter((e: ProcessedEvidenceItem) => e.verified).length;
        }

        const scottLogsStr = localStorage.getItem('scottLogs');
        if (scottLogsStr) {
          daysDenied = JSON.parse(scottLogsStr).filter((l: ScottLogEntry) => l.category === 'Denial of Parenting Time').length;
        }

        const auditLogsStr = localStorage.getItem('systemAuditLogs');
        if (auditLogsStr) {
          auditTargets = JSON.parse(auditLogsStr).length;
        }

        const medicalRecordsStr = localStorage.getItem('medicalRecords');
        if (medicalRecordsStr) {
          sjrhPages = JSON.parse(medicalRecordsStr).reduce((sum: number, r: MedicalRecord) => sum + (r.pageCount || 0), 0);
        }
