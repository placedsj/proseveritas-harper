...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const [custodyHearing, setCustodyHearing] = useState<CourtEvent | null>(null);
  const [sentencingHearing, setSentencingHearing] = useState<CourtEvent | null>(null);
  ...
  useEffect(() => {
    ...
    setCustodyHearing(nextCustody);
    setSentencingHearing(nextSentencing);

  }, []);

  useEffect(() => {
    // TIMERS FOR HEADERS
    // Fallback dates if no events found
    const targetDate = custodyHearing ? new Date(`${custodyHearing.date}T09:30:00`) : new Date('2026-03-30T09:30:00');
    const sentencingDate = sentencingHearing ? new Date(`${sentencingHearing.date}T09:00:00`) : new Date('2026-03-03T09:00:00');
  ...
  }, [custodyHearing, sentencingHearing]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex items-center gap-4 w-full mb-4">
               ...
               <div className="text-left">
                   <h2 className="text-xl font-black text-white uppercase tracking-widest leading-none">
                     {custodyHearing ? custodyHearing.requiredAction : 'Custody Hearing'}
                   </h2>
                   <p className="text-indigo-300 text-xs font-semibold mt-1">
                     {custodyHearing ? `${custodyHearing.date} (Judge ${custodyHearing.judgeName})` : 'Date Pending'}
                   </p>
               </div>
          </div>
        ...
        <div className="bg-gradient-to-br from-blue-900 to-slate-900 border border-blue-700 rounded-xl p-6 flex flex-col items-center justify-between">
          <div className="flex items-center gap-4 w-full mb-4">
               ...
               <div className="text-left">
                   <h2 className="text-xl font-black text-white uppercase tracking-widest leading-none">
                     {sentencingHearing ? sentencingHearing.requiredAction : 'Sentencing'}
                   </h2>
                   <p className="text-blue-300 text-xs font-semibold mt-1">
                     {sentencingHearing ? `${sentencingHearing.date} (Judge ${sentencingHearing.judgeName})` : 'Date Pending'}
                   </p>
               </div>
          </div>
