...
const MedicalRecords: React.FC = () => {
  ...
  const handleAddRecord = () => {
    ...
    setIsAdding(false);
    setNewRecord({ title: '', source: '', dateOfRecord: new Date().toISOString().split('T')[0], ocrText: '', status: 'needs_review', pageCount: 0 });
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-xl border border-blue-200 animate-fade-in shadow-lg text-left">
           <h3 className="text-lg font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2 uppercase tracking-wider">New Medical Record</h3>

           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
              <div>
                  <label className="block text-slate-500 text-xs uppercase font-bold mb-1">Title</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-slate-500 text-xs uppercase font-bold mb-1">Date of Record</label>
                    <input
                      type="date"
           ...
           <div className="mb-4">
              <label className="block text-slate-500 text-xs uppercase font-bold mb-1">Source</label>
              <input
                type="text"
           ...
           <div className="mb-6">
              <label className="block text-slate-500 text-xs uppercase font-bold mb-1">OCR Text / Content</label>
              <textarea
                value={newRecord.ocrText}
                onChange={e => setNewRecord({...newRecord, ocrText: e.target.value})}
                className="w-full bg-slate-50 border border-slate-300 rounded p-3 text-slate-900 focus:border-blue-500 focus:outline-none h-32 font-mono text-xs"
           ...
           <button
             ...
             className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded font-bold shadow-md transition-all flex justify-center items-center gap-2 uppercase tracking-widest"
           >
