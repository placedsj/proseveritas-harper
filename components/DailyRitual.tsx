  ...
  <div className="relative group w-full">
    <button
      ...
      className={`w-full text-left flex items-start gap-4 p-4 rounded-lg transition-colors ${
        checked ? 'bg-green-900/20 border border-green-800' : 'bg-slate-800 border border-slate-700 group-hover:bg-slate-750'
      }`}
    >
    ...
    {audioEnabled && (
      <button
        ...
        className="absolute right-3 top-3 p-2 text-slate-500 hover:text-blue-400 opacity-50 group-hover:opacity-100 transition-all z-10"
        title="Play Reminder"
        aria-label={`Play reminder for ${label}`}
      >
